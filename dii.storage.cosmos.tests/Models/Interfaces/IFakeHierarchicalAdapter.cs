...
namespace dii.storage.cosmos.tests.Models.Interfaces
{
    public interface IFakeHierarchicalAdapter<T> where T : IDiiEntity, new()
    {
        ...
        Task<List<T>> GetManyAsync(IReadOnlyList<(string id, Dictionary<string, string> partitionKeys)> idAndPks, ReadManyRequestOptions readManyRequestOptions = null, CancellationToken cancellationToken = default);
