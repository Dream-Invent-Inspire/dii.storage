...
namespace dii.storage.cosmos.tests.Models.Interfaces
{
    public interface IFakeAdapter<T> where T : IDiiEntity, new()
    {
        ...
        Task<List<T>> PatchBulkAsync(IReadOnlyList<(string id, string partitionKey, IReadOnlyList<PatchOperation> listOfPatchOperations)> patchOperations, PatchItemRequestOptions requestOptions = null, CancellationToken cancellationToken = default);
